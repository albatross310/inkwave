...
// ⚠ ONE CANONICALISER, TWO CALLERS. `canonicalize` lives in `./jcs.mjs`, which the signing server
// (`api/_provenance-core.mjs`) imports too; the signature is over its output. Re-exported here so
// this module's public API is unchanged. `jcs.test.ts` pins every byte from both entry points.
export { canonicalize } from './jcs.mjs'
