...
//
// MERGED 2026-07-17: these are `feat/prod-graphs`' tests, verbatim. They covered the five time
// functions its placeholder `ledger.ts` mirror carried; that mirror is retired (the real
// `ledger.ts` is the prod-ledger lane's per-month attested ledger) and the functions now live in
// `sessionLogic.ts`, so only the import moved. Not one assertion changed.
//
...
import { localDayOf, localHourOf, monthOf, weekStartOf, weekdayOf } from './sessionLogic'
