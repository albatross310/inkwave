...
export function syncPrintPageStyle(): void {
  ...
  // (.inkwave-sheet-num — EB Garamond 0.9rem #9b5ccc beside the 22px mark). Falls back to the
  ...
  const chrome = logoUri
    ? `@bottom-center{content:url("${logoUri}") " " counter(page);` +
      `font-family:'EB Garamond',Georgia,serif;font-size:0.9rem;color:#9b5ccc;margin-bottom:16px;}`
    : ''
  const css =
    `@media print{` +
