...
// Everything the loading animation will ever do is computed BEFORE playback; playback is a pair
// of WAAPI tracks per instance plus the tiles' CSS drift.
// → docs/archive/editor-surface.md#twinkle-model
//
//   1. ⚠ NOTHING RUNS PER FRAME ON THE MAIN THREAD DURING THE LOAD — no rAF driver, no respawns,
//      no style writes — so main-thread starvation is PHYSICALLY INCAPABLE of touching it. That
//      is the whole design: anything new on the load path must be precomputed.
//   2. ART IS CANVAS-RASTERED PNG, NEVER an SVG data URI, and only at pool build time.
//   3. ONE CLOCK, SET ONCE: every track's startTime ≡ the tile drift's startTime (mod the 1944ms
//      tile loop), batch-stamped by `alignTracks` once per load. No epoch globals, no per-frame
//      re-anchoring, no resync — and never create-then-re-clock on VISIBLE water (`clockReady`).
//   4. AN INSTANCE RIDES ITS WAVE WITH ZERO MAINTENANCE: its transform track bakes the 72px/s
//      drift, so a lit envelope holds a CONSTANT wave-space position and the dark window glides
//      it to the next slot on its OWN 140px lattice (same phase ⇒ the same art stays valid),
//      drawn through the never-strike-twice band memory.
//   5. THE FIELDS TAKE THE SAME INJECTED BRAKE as the tiles (Scroll.tsx's additive coast), so
//      every layer decelerates in lockstep with the water, continuous by construction.
//   6. ⚠ RELOCATION ON THE SCROLL PATH MUST NEVER RASTERISE — 140px-lattice moves only. Sway is
//      LITERAL field transforms (`swayFields`), never a `var(--wave-x)` read: see the firebreak.
...
// ─── Art rasteriser — canvas PNG data-URIs, NEVER SVG data-URIs ──────────────────────────────
// ⚠ Chromium builds an ISOLATED SVG DOCUMENT per unique SVG-image URI — traced at ~4.3s of boot
// at ~600 URIs (Firefox's path is cheap, hence the asymmetry). A canvas raster of the same
// strokes decodes off-thread with no document. One shared canvas, and ONLY at pool build time.
...
function alignTracks(): void {
  ...
  const apply = () => {
    ...
    // ⚠ Batch re-clock of anims created BEFORE the clock resolved — legal ONLY because the
    // pre-gate boot mounts are display-hidden until the recalc that starts the drift. On VISIBLE
    // water, creation is gated on `clockReady()`: a late batch re-clock of live marks is a MASS
    // TELEPORT of the whole field (+24-62px phase steps, measured).
    // → docs/archive/editor-surface.md#twinkle-clock-gate
...
// Entry fade for mark tracks created on VISIBLE water: hold the layer at 0, create, then fade in
// — deferred creation must never pop a whole layer in one frame. ON THE FIELD, not the blink
// wrapper: the coast's fade ANIMATION overrides a wrapper's inline opacity, while the field only
// ever animates TRANSFORM. → docs/archive/editor-surface.md#twinkle-clock-gate
...
// ─── Field transforms ─────────────────────────────────────────────────────────────────────────
// ⚠ During the LOAD a field carries NO inline transform — the coast brake is pure CSS, the same
// injected keyframes the tiles composite. At REST it takes LITERAL sway transforms, never
// `var(--wave-x)`: a var-consuming field puts the whole instance subtree inside the sway's
// invalidation set. → docs/archive/editor-surface.md#twinkle-model
...
function mountSet(host: HTMLElement, h: HostState, kind: 'sparks' | 'dashes'): void {
  ...
  if (waterMode === 'off') {
    ...
    // A LATER host (the covered editor under the shell) is invisible until the reveal, so its
    // ~330 track animations (~250ms) are built in CHUNKED idle slices and never sit on the boot's
    // critical path. Clock-exact either way. → docs/archive/editor-surface.md#twinkle-later-host
    let cursor = 0
    const slice = () => {
...
function startRestDrift(hostEl: HTMLElement, nodes: SetNodes): void {
  // ⚠ The rest ramp is NON-WRAPPING and anchored to its field's CSS brake's literal startTime —
  // the same clock Scroll's resolve stamps — so the rest commit swaps in identical pixels BY
  // CONSTRUCTION. A looping 140px ramp teleports the lit statics at every wrap, and an anchor
  // taken at settle-time has a wrap ambiguity against the resolve; both were real.
  // → docs/archive/editor-surface.md#twinkle-rest-drift
  ...
  for (const g of ['a', 'b'] as Group[]) {
    ...
    if (typeof brake.startTime === 'number') start()
      ...
      // Pending until the forward anchor plays it: `ready` + a poll fallback, whichever lands
      // first (start() is idempotent). The anchor stays EXACT however late we read it, because
      // t0c is the literal startTime.
...
function enterRest(): void {
  // ONE commit, in the same flush as the surface's wave classes dropping: cancel the load
  // playback, stop the rest drift, and put the fields on literal sway transforms at the tiles'
  // handed-off --wave-x. → docs/archive/editor-surface.md#twinkle-rest-commit
...
export function syncTwinkles(
  ...
  if (want.mode === 'anim' && prev !== 'anim') {
    ...
    // load's clock exists: creating them on a provisional clock and re-timing later teleports
    // every visible mark at once. → docs/archive/editor-surface.md#twinkle-clock-gate
