...
export function TiptapEditor({ doc, onDocChange, onDuplicateEmail }: TiptapEditorProps) {
  ...
  // Toolbar customisation slots — the row, both drags, the write-back and the positional hotkeys
  // live in useToolbarSlots.ts. ⚠ CALLED HERE, where the block it replaced began, so its effects
  // keep their place in this component's effect order (R7). What is OPEN stays below: the drawer
  // look and the honeycomb's column count are derived from panel state the hook does not hold.
  const {
  ...
  } = useToolbarSlots({ docRef, commitDoc, closeDrawer: () => setPanelOpen('drawer', false) })
