...
function Harness({ doc, log, phone }: { doc: InkwaveDocument; log: Log; phone: boolean }) {
  ...
  // ⚠ THE DRAWER'S OPEN STATE IS THE HARNESS'S, NOT THE HOOK'S (2026-09-19). PR #9's hook owned the
  // drawer's open flag; on master that value is DERIVED from panel state the hook does not hold, so
  // the component owns it and the hook is told only how to CLOSE the drawer after a drop. This
  // harness stands in for the component and does the same, which is why these few lines differ from
  // #9's copy of this file. Every ASSERTION below is unchanged from #9.
  const [pickerOpen, setPickerOpen] = useState(false)
  const api = useToolbarSlots({ docRef, commitDoc, closeDrawer: () => setPickerOpen(false) })
  const isTouch = phone
  ...
  return (
    <div className="iw-toolbar-circles" onClickCapture={(e) => {
      ...
      <div className="relative" ref={api.toolbarPickerRef}>
        <button type="button" data-test="picker" onClick={() => { log.clicks.push('▲'); setPickerOpen(o => !o) }} />
        <div data-test="drawer" className={pickerOpen ? '' : 'invisible'}>
          {overflowSlots(api.toolbarSlots).map(id => (
            <div key={id} className="iw-slot" onClick={() => setPickerOpen(false)}
      ...
      <output data-test="state">{JSON.stringify({
        ...
        active: api.popupDragActive, open: pickerOpen, alt: api.altHeld,
      })}</output>
    </div>
  )
}
...
// ⚠ #9'S OUTSIDE-CLICK TEST IS NOT HERE, AND THAT IS NOT A DELETION (2026-09-19). It asserted that
//   a mousedown outside the ▲ closes it, as a HOOK effect. Two things about master make it belong
//   elsewhere. The drawer's open state is the component's now, not the hook's — so the hook has no
//   flag to clear. And rule (a) closes panels on **pointerdown**, never mousedown, deliberately:
//   iOS withholds the synthetic mouse event under the touch guard's preventDefault. So #9's test
//   would be stale about the event even if the state had stayed. The behaviour is guarded in two
//   places instead, both of which a reviewer should check rather than take on trust:
//   `tapClosesPanel` in toolbarContract.test.ts (the decision), and a source-scan guard in
//   toolbarSlotsWiring.test.ts (that the component still wires it, on pointerdown, with the
//   main-row exemption that lets a drag out of the row begin).
