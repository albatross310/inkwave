...
const HIGHLIGHT_FILL: Record<HighlightKind, string> = {
  scas: 'rgba(65, 66, 91, 0.28)',   // --iw-light purple, SCAS kicked word
...
export function paintPage(
  ...
  for (let i = 0; i < model.lines.length; i++) {
    ...
    for (const seg of line.segs) {
      ...
      if (seg.atom) {
        if (seg.w <= 0) continue
        ctx.fillStyle = seg.atom === 'citation' ? 'rgba(48, 36, 56, 0.55)' : 'rgba(48, 36, 56, 0.30)'
  ...
  for (let p = 0; p < model.pages; p++) {
    ...
    for (let i = 0; i < model.lines.length; i++) {
      ...
      if (block.kind === 'placeholder' && block.type !== 'paragraph') {
        ...
        ctx.fillStyle = 'rgba(48, 36, 56, 0.18)'
