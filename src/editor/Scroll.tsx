...
export function Scroll({
  ...
  // transform-magnify + fit-to-width cap. Phone has its own model; SnapshotView's in-flow Scroll
  // and 'scroll' paper (no mm width) stay plain.
  ...
  // Stochastic twinkles (see waveTwinkle.ts). ⚠ The container div is EMPTY in the JSX and the
  // prerender, and instances are populated CLIENT-ONLY after hydration, so server HTML and the
  // first client render always match. ⚠ useLAYOUTeffect deliberately: the mode handoffs must land
  // in the SAME pre-paint flush as the wave class swap, or one visible frame shows the dashes
  // standing still against moving waves. → docs/archive/editor-surface.md#scroll-chrome
  ...
  useLayoutEffect(() => {
    ...
    // No clock/travel plumbing: the fields' brake is the SAME injected keyframes the tiles
    // composite (one writer), and the pool's clock is aligned once per load inside waveTwinkle.
    syncTwinkles(host, {
      sparks: waveMode !== 'off',
