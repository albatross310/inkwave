...
//
// WHAT THIS FILE IS (docs/REFACTOR-QUEUE.md item 3, seam 1): the writer's arrangement of the footer
// toolbar's slot row — which six circles it holds and in what order, every way the writer rearranges
// it (phone touch-hold reorder, touch-hold drag from the ▲ drop-up onto the row), the write-back to
// the document and to the writer's own default, and the POSITIONAL hotkeys that address that row by
// index. The circles themselves (the footer JSX) stay in TiptapEditor.tsx, which attaches what this
// hook hands back; `toolbarSlotsWiring.test.ts` pins that wiring. Behaviour is pinned in
// `useToolbarSlots.test.tsx`; the in-browser truth is `scripts/toolbar.prove.mjs`.
//
// ⚠ WHAT IS DELIBERATELY *NOT* HERE, AND WHY THE SEAM MOVED (2026-09-19). PR #9 extracted this
//   block in September and also took `toolbarPickerOpen` with it as hook state. Master has since
//   made that value DERIVED — from `openPanel`, `activeBar` and the row's own contents (the phone
//   rule that an app opened from the drawer keeps the drawer look until it closes). A hook cannot
//   own a value computed from panel state it does not hold, so `toolbarPickerOpen`, `drawerHeld`
//   and the desktop honeycomb's column count stay in the component. This hook owns the ROW and how
//   the writer rearranges it; the component owns what is OPEN. That is the line, and it is the
//   reason this is a re-extraction rather than a rebase of #9.
//
...
// position the block occupied, so its effects keep their place in the component's effect sequence.
// Do not move the call.
import {
  useEffect, useRef, useState,
  type MutableRefObject, type RefObject, type TouchEvent as ReactTouchEvent,
} from 'react'
...
/** Hold before either drag arms. A real drag begins with stillness; see `brokeHoldSlop`. */
const HOLD_MS = 400
...
export interface ToolbarSlotsInput {
  ...
  /**
   * Close the ▲ drawer after a drop-up entry is dragged onto the row. The component owns panel
   * state (see the seam note above), so the hook asks rather than sets: it must not learn what
   * `openPanel` is in order to close one drawer.
   */
  closeDrawer: () => void
}
...
export interface ToolbarSlots {
  ...
  /** Desktop HTML5 drag: the slot id currently being dragged. */
  dragIdRef: MutableRefObject<SlotId | null>
  /** A drop just happened — swallow the click the release would otherwise fire. */
  suppressSlotClickUntilRef: MutableRefObject<number>
  /** Alt has been held DELIBERATELY (not as part of a chord) — the row shows its hint badges. */
  altHeld: boolean
  /** Live phone reorder, or null. */
  slotDragView: SlotDragView | null
  slotTouchHandlers: (slotIdx: number) => SlotTouchHandlers
  /** The row index a dragged ▲ entry is hovering, or null. */
  popupDragTarget: number | null
  /** A ▲ entry is armed and following the finger. */
  popupDragActive: boolean
  popupTouchHandlers: (id: SlotId) => SlotTouchHandlers
}

export function useToolbarSlots({ docRef, commitDoc, closeDrawer }: ToolbarSlotsInput): ToolbarSlots {
  ...
  const [slotDragView, setSlotDragView] = useState<SlotDragView | null>(null)
  ...
  return {
    toolbarSlots, updateSlots, toolbarPickerRef, slotElsRef, dragIdRef,
    suppressSlotClickUntilRef, altHeld, slotDragView, slotTouchHandlers,
