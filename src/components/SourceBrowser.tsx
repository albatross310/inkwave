...
// a literal `#302438` left on the night HEADER measured 1.13:1 (invisible), and chrome tokens
...
const INKC = 'var(--iw-ink, #302438)'                     // purple inside a floating chrome bubble
const CHROME_FG = 'var(--iw-reader-chrome-fg, #302438)'   // the header strip's own ink
const CHROME_DIM = 'var(--iw-reader-chrome-dim, #d6d3d1)' // …and its disabled glyphs
const INKP = 'var(--iw-reader-accent, #302438)'           // purple ON reader paper / the markup bar
...
const OPEN_TAB_FILL = `linear-gradient(135deg, #41425b, ${INK})`
const HAIR = 'var(--iw-reader-hair, rgba(48, 36, 56, 0.13))'// hairline rules/dividers on paper
const TINT = 'var(--iw-reader-tint, rgba(48, 36, 56, 0.08))'// the lit-tool fill
  ...
  const extensionOffer = (): React.ReactNode => {
    ...
    return (
      <div style={{ maxWidth: 470, width: '100%', textAlign: 'left', borderRadius: 10,
        border: '1px solid var(--iw-nightable-border, rgba(48, 36, 56, 0.28))', background: `${INK}0a`,
        ...
        {!extHow ? (
          <button type="button" onClick={() => setExtHow(true)} data-iw-ext-how
            ...
            style={{ marginTop: 8, border: '1px solid var(--iw-nightable-border, rgba(48, 36, 56, 0.33))', color: CHROME_FG, fontSize: '12px', background: 'transparent', cursor: 'pointer' }}>
            How to install it
          </button>
        ) : (
          <>
    ...
    <div className="iw-nightable iw-touch-guard iw-dock-panel flex flex-col bg-white"
      ...
      <div className="flex flex-col flex-1 min-h-0">
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-stone-200"
          ...
          {/* ⚠ THE ENABLED ARROW WAS THE LITERAL #302438 ON THE NIGHT HEADER — 1.13:1, invisible,
          ...
          <input value={addr} onChange={(e) => setAddr(e.target.value)}
            ...
            style={{ flex: 1, minWidth: 80, height: isPhone ? TOUCH_FIELD_H : 22, fontSize: '11px', padding: '0 7px', borderRadius: 999,
              border: '1px solid var(--iw-nightable-border, rgba(48, 36, 56, 0.28))', background: 'transparent',
              ...
              <div className="flex flex-col items-center justify-center gap-3 h-full text-center px-8" style={{ fontSize: '14px', color: 'var(--iw-pill-fg, #57534e)' }}>
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={onClose}
                    className="rounded-full px-3 py-1.5" style={{ border: '1px solid var(--iw-nightable-border, rgba(48, 36, 56, 0.33))', color: framed ? CHROME_FG : INKP, fontSize: '13px' }}>
                    Close this panel
                  </button>
                ...
                <div className="flex gap-2 flex-wrap justify-center">
                  {extState === 'blocked' && (
                    <button type="button" onClick={askForFetchPermission}
                      className="rounded-full px-3 py-1.5" style={{ border: '1px solid var(--iw-nightable-border, rgba(48, 36, 56, 0.33))', color: CHROME_FG, fontSize: '13px' }}>
        ...
        {composer && !framed && (
          <div ref={composerBoxRef} className="iw-nightable fixed z-[402] flex items-center gap-1.5 bg-white rounded-full shadow-lg px-2 py-1.5"
            style={{ left: composer.x, top: Math.max(8, composer.y - 46), transform: 'translateX(-50%)',
              ...
              border: '1px solid var(--iw-nightable-border, rgba(48, 36, 56, 0.28))', fontSize: '12px', ['--iw-tap-x' as string]: '6px' }}>
            {TEXT_COLORS.map((c) => (
        ...
        {sel && !framed && (onCite || onQuote) && (
          <div ref={selBoxRef} className="iw-nightable fixed z-[401] flex items-center gap-1 bg-white rounded-full shadow-lg px-1.5 py-1"
            style={{ left: sel.x, top: Math.max(8, sel.y - 42), transform: 'translateX(-50%)',
              maxWidth: 'calc(100vw - 16px)', border: '1px solid var(--iw-nightable-border, rgba(48, 36, 56, 0.28))', fontSize: '12px', ['--iw-tap-x' as string]: '4px' }}
            onMouseDown={(e) => e.preventDefault()}>
            ...
            <span style={{ width: 1, height: 16, background: 'var(--iw-nightable-border, rgba(48, 36, 56, 0.28))', margin: '0 2px' }} />
            {onQuote && (
        ...
        {toast && (
          <div style={{ position: 'absolute', left: '50%', bottom: 54, transform: 'translateX(-50%)', zIndex: 402,
            background: 'var(--iw-ink, #302438)', color: 'var(--iw-on-ink, #fff)', fontSize: '12px', padding: '6px 12px', borderRadius: 999,
