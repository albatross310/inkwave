...
// ─── THE TYPE RAMP ───────────────────────────────────────────────────────────────────────────
// Peter, 2026-07-17: "the entire text font of the panel needs to be increased. It's okay if users
// have to scroll." / "Every font proportionally up."
//
// This IMPORTS the existing ramp rather than picking new numbers. The `music/` path is a misnomer
// — the module is a general five-step semantic ramp and its own header quotes THIS panel as the
// instruction it was built from ("Music likewise needs all the fonts increased"). A second ramp
// here would be exactly what it exists to prevent: nine nearly-identical sizes, accumulated one
// component at a time. If it ever moves to a neutral path, this import follows; it must not fork.
//
// The steps are SEMANTIC. `TYPE.body` for anything the writer types INTO is also what keeps the
// iOS auto-zoom trap unreachable (every step is ≥16) — this file previously shipped 13px inputs,
// which iOS zooms into on focus AND STAYS zoomed, mid-sentence, on the writer's own phone.
    ...
    <>
      <section className="px-4 py-3" style={{ borderTop: '1px solid var(--iw-nightable-border, #f0eeec)' }}>
        ...
        {goals.length === 0 && (
          ...
          <p className="mb-2 leading-relaxed" style={{ fontSize: TYPE.body, color: 'var(--iw-pill-fg, #a8a29e)' }}>
        ...
        <div className="mt-2 flex gap-1.5">
          ...
          <button
            ...
            style={{ fontSize: TYPE.body, minWidth: TOUCH_MIN, minHeight: TOUCH_MIN, background: 'var(--iw-ink, #5c2d8a)', color: 'var(--iw-on-ink, #fff)' }}
          >
            +
          </button>
        </div>
      ...
      <section className="px-4 py-3" style={{ borderTop: '1px solid var(--iw-nightable-border, #f0eeec)' }}>
        <h3 className="mb-1.5 uppercase tracking-wider" style={{ fontSize: TYPE.label, color: 'var(--iw-pill-fg, #a8a29e)' }}>The rough plan</h3>
        <textarea
          value={plan}
        ...
        />
        <p className="mt-1.5 leading-relaxed" style={{ fontSize: TYPE.meta, color: 'var(--iw-pill-fg, #a8a29e)' }}>
    ...
    return (
      <li className="flex gap-1.5">
        ...
        <input
          ...
          style={{ fontSize: TYPE.meta, minHeight: TOUCH_MIN, border: '1px solid var(--iw-nightable-border, #e7e5e4)', background: 'transparent', color: 'var(--iw-pill-fg, #78716c)' }}
        />
        <button type="button" onClick={save} className="shrink-0 px-1.5" style={{ fontSize: TYPE.body, minWidth: TOUCH_MIN, minHeight: TOUCH_MIN, color: 'var(--iw-ink, #5c2d8a)' }}>✓</button>
      </li>
    )
  }
  ...
  return (
    <li className="group flex items-baseline gap-2">
      <button
        ...
        style={{ fontSize: TYPE.body, minWidth: TOUCH_MIN, minHeight: TOUCH_MIN, color: done ? 'var(--iw-verified, #15803d)' : 'var(--iw-pill-fg, #a8a29e)' }}
      >
        {done ? '☑' : '☐'}
      </button>
      <div className="min-w-0 flex-1">
        <button type="button" onDoubleClick={() => setEditing(true)} className="block w-full truncate text-left"
          title="Double-click to edit"
          style={{
            fontSize: TYPE.body,
            color: done ? 'var(--iw-pill-fg, #a8a29e)' : 'var(--iw-ink, #5c2d8a)',
            textDecoration: done ? 'line-through' : undefined,
          }}
        >
          {goal.text}
        </button>
        <span style={{ fontSize: TYPE.meta, color: line.tone === 'good' ? 'var(--iw-verified, #15803d)' : 'var(--iw-pill-fg, #a8a29e)' }}>
      ...
      <button
        ...
        style={{ fontSize: TYPE.body, minWidth: TOUCH_MIN, minHeight: TOUCH_MIN, color: 'var(--iw-pill-fg, #a8a29e)' }}
      >
        ×
      </button>
    </li>
