          ...
          <div className="flex items-end gap-2">
            <label className="flex flex-col gap-1 flex-1">
              ...
              <select value={type} onChange={e => setType(e.target.value)}
                className="text-sm border border-stone-200 rounded px-2 py-2 bg-white focus:outline-none focus:border-[#302438]">
              ...
              <label key={f.key} className={`flex flex-col gap-0.5 min-w-0 ${wide ? 'col-span-2' : ''}`}>
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    className={`flex-1 text-sm border rounded px-2 py-2 focus:outline-none ${missing ? 'border-red-200 focus:border-red-400' : 'border-stone-200 focus:border-[#302438]'}`}
                  />
                  {fetchable && (
                    <button type="button" onClick={() => void fetchFields(f.key as 'DOI' | 'URL')}
                      ...
                      className="flex-shrink-0 w-9 h-9 rounded border border-stone-200 text-stone-500 hover:border-[#302438] hover:text-[#302438] disabled:opacity-40 flex items-center justify-center text-base">
          ...
          <div className="px-4 py-2.5 border-b border-stone-100 flex items-center justify-between"
            ...
            <div className="flex gap-2 flex-shrink-0 ml-3 items-center">
              <a href="https://chromewebstore.google.com/detail/inkwave-citation-capture/TODO"
                ...
                className="text-xs text-stone-500 hover:text-[#302438] underline underline-offset-2">
                Chrome
              </a>
        ...
        <div className={isTouchDevice() ? 'order-3 px-4 pt-2 pb-3 border-t border-stone-100' : 'px-4 pt-3 pb-2 border-b border-stone-100'}>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 text-sm border border-stone-200 rounded px-3 py-2 focus:outline-none focus:border-[#302438]"
            />
            <button
              type="button" disabled={!captureable || busy} onClick={() => void doCapture()}
              className="text-sm px-3.5 py-1.5 rounded disabled:opacity-40"
              style={{ background: 'var(--iw-addbtn-bg, #302438)', color: 'var(--iw-addbtn-fg, #fff)', border: '1px solid var(--iw-addbtn-border, transparent)' }}
            >
              {busy ? '…' : 'Add'}
            </button>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {(['cited', 'all', 'manual'] as RefMode[]).map(m => (
              <button
                ...
                style={refMode === m
                  ? { background: `${INK}12`, borderColor: 'var(--iw-cite-color, #302438)', color: 'var(--iw-cite-color, #302438)' }
            ...
            <button type="button" onClick={toggleSortDir}
              ...
              className="h-8 w-8 flex items-center justify-center rounded border border-stone-200 text-stone-500 hover:border-[#302438] hover:text-[#302438]">
              {sortDir === 'asc' ? '↑' : '↓'}
            </button>
            ...
            <div className="relative flex-shrink-0" onMouseEnter={() => setRecheckTip(true)} onMouseLeave={() => setRecheckTip(false)}>
              <button type="button" onClick={() => void recheckAll()} disabled={recheckingAll}
                className="h-8 w-8 flex items-center justify-center rounded border border-stone-300 disabled:opacity-50 hover:bg-[#3024380d]"
                style={{ color: 'var(--iw-pill-fg, #302438)' }}>
                {recheckingAll ? '…' : '↻'}
              </button>
              {recheckTip && (
          ...
          {!helpDismissed && (
            <div className={`flex items-start gap-2 mb-2 ${fullscreen ? 'col-span-2' : ''}`}>
              <div className="text-[11px] text-stone-500">
                type <kbd className="font-mono border border-stone-200 rounded px-0.5" style={{ background: 'var(--iw-chip-bg, #f5f5f4)' }}>@</kbd> in the editor to insert · <span className="text-[#302438]">📎</span> embed a PDF, then <span className="text-[#302438]">📄</span> next to an in-text citation opens it at the cited page.
              </div>
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0 max-w-[48%]">
                      ...
                      return iw?.pdfName ? (
                        <>
                          <button type="button"
                            ...
                            className="text-[11px] px-2 py-0.5 rounded border border-[#30243855] text-[#302438] hover:border-[#302438] hover:bg-[#3024380d] whitespace-nowrap"
                        ...
                        <button type="button"
                          ...
                          className="text-[11px] px-2 py-0.5 rounded border border-stone-200 text-stone-500 hover:border-[#302438] hover:text-[#302438] whitespace-nowrap"
                   ...
                   <div className="flex items-center justify-end gap-1">
                    <button type="button" onClick={e => { e.stopPropagation(); cite(item) }} title="Cite inline in the document at the cursor"
                      className="w-7 h-7 flex items-center justify-center rounded border border-stone-200 text-stone-500 hover:border-[#302438] hover:text-[#302438] text-[15px] leading-none">@</button>
                    {!!(item.URL || (item as { _iw?: IwCitationMeta })._iw?.sourceUrl) && (
                      <button type="button"
                        title="Open source page (shows verification panel if extension installed)"
                        className="w-7 h-7 flex items-center justify-center rounded border border-stone-200 text-stone-500 hover:border-[#302438] hover:text-[#302438] text-[13px]"
                      ...
                      <div className="mt-0.5 pl-1 flex items-center gap-2">
                        ...
                        {changelog.length > 0 && (
                          <button type="button" onClick={e => { e.stopPropagation(); toggleExpand(item.id) }}
                            ...
                            className="text-[12px] px-1 rounded border border-stone-200 text-stone-500 hover:border-[#302438] hover:text-[#302438] flex-shrink-0">
                        ...
                        <div className="mt-1 ml-1 border-l-2 border-stone-100 pl-2 space-y-1">
                          {changelog.map((c, i) => (
                            <div key={i} className="text-[12px] text-stone-500 flex items-center gap-1.5">
                              ...
                              <span className="truncate max-w-[80px]" style={{ color: 'var(--iw-cite-color, #302438)' }}>{fmtVal(c.field, c.new)}</span>
                              <span className="text-[11px] text-stone-300 flex-shrink-0">{c.source}</span>
                              <button type="button" onClick={() => void revert(item, i)}
                                className="text-[11px] text-stone-500 hover:text-[#302438] ml-auto flex-shrink-0">revert</button>
                            </div>
                          ))}
                        </div>
                      )}
