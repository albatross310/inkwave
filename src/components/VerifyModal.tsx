...
function Row({ label, ok, detail }: { label: string; ok?: boolean; detail?: string }) {
  return (
    <div className="flex items-baseline gap-3 py-1.5 border-b" style={{ borderColor: '#eee' }}>
      ...
      <span className="w-36 shrink-0 text-sm" style={{ color: 'var(--iw-ink, #302438)' }}>{label}</span>
...
function Stat({ label, value, hint }: { label: string; value: React.ReactNode; hint?: string }) {
  return (
    <div>
      <div className="text-2xl leading-none" style={{ color: 'var(--iw-ink, #302438)' }}>{value}</div>
