...
const FAMILIES = [
  ...
  // GPT-adjacent screen sans options (2026-09-07). Appended so regenerating the font directory
  // preserves every existing generated filename; both stay on-demand (shouldPreload remains EB
  // Garamond only), so adding picker choice does not add bytes to the ordinary page load.
...
// Only the immediately painted default face belongs on the critical path. Every other weight,
// style, alphabet and optional family remains self-hosted and is fetched by @font-face on demand.
const shouldPreload = (family, weight, style, subset) =>
  family === 'EB Garamond' && weight === '400' && style === 'normal' && subset === 'latin'
...
// CANONICAL FONT BYTES ARE PINNED. Google can reorder its CSS response (adding Noto/Open moved
// Spectral + Zilla by 72 global indices) and can update a face behind the same family request. The
// old generator wiped/redownloaded everything, silently changing metrics in established documents;
// one such refresh made the already-shipped Zilla fail WebKit canvas parity. Existing families are
// therefore carried BYTE-FOR-BYTE with their existing @font-face blocks. Only a family absent from
// the hosted CSS is downloaded. An intentional family refresh must be a separate measured migration,
// never a side effect of adding another picker option.
const priorCss = (() => { try { return fs.readFileSync('public/fonts/inkwave-fonts.css', 'utf8') } catch { return '' } })()
const faceRe = /@font-face\s*\{([\s\S]*?)\}/g
const existingFamilies = new Set()
const keptAssets = new Map()
let priorFace
while ((priorFace = faceRe.exec(priorCss))) {
  const family = (priorFace[1].match(/font-family:\s*'([^']+)'/) || [])[1]
  if (family) existingFamilies.add(family)
  const name = (priorFace[1].match(/url\(\/fonts\/([^)]+)\)/) || [])[1]
  if (name && !keptAssets.has(name)) {
    try { keptAssets.set(name, fs.readFileSync(`public/fonts/${name}`)) } catch { /* verification below refuses the gap */ }
  }
}
const priorPreload = (() => { try { return fs.readFileSync('app/fontPreload.ts', 'utf8') } catch { return '' } })()
fs.rmSync('public/fonts', { recursive: true, force: true })
fs.mkdirSync('public/fonts', { recursive: true })
for (const [name, bytes] of keptAssets) fs.writeFileSync(`public/fonts/${name}`, bytes)

// Each @font-face is preceded by a `/* <subset> */` comment in the Google css2 output.
const re = /(?:\/\*\s*([\w-]+)\s*\*\/\s*)?@font-face\s*\{([\s\S]*?)\}/g
let out = priorCss.trim()
  || '/* Inkwave self-hosted fonts — generated by scripts/fetch-fonts.mjs. Do not edit by hand.\n   Same-origin so PDF/print render is deterministic (no external fetch race). */'
const preload = []
let m
const familyFaceIndex = new Map()
let bytes = 0
while ((m = re.exec(css))) {
  const [, subset, body] = m
  ...
  const index = familyFaceIndex.get(fam) ?? 0
  familyFaceIndex.set(fam, index + 1)
  const name = `${slug}-${weight}-${style}-${index}.woff2`
  const buf = Buffer.from(await (await fetch(src, { headers: { 'User-Agent': UA } })).arrayBuffer())
  fs.writeFileSync(`public/fonts/${name}`, buf)
  bytes += buf.length
  out += '\n\n@font-face {' + body.replace(src, `/fonts/${name}`) + '}'
  if (shouldPreload(fam, weight, style, subset)) preload.push(`/fonts/${name}`)
}
...
if (priorPreload) fs.writeFileSync('app/fontPreload.ts', priorPreload)
...
console.log(`verified ${check.checked} hosted faces; added ${familyFaceIndex.size} family/families (${(bytes / 1024).toFixed(0)} KB); existing bytes preserved`)
