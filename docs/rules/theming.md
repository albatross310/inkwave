...
- **Component tests work** (vite.config.ts drops the React Router plugin under vitest).
  ...
  build recorders inside the factory from plain functions. Third (2026-09-15,
  `useToolbarSlots.test.tsx`): under `vi.useFakeTimers()` wrap every timer advance in `act()` or the
  state it flips never commits, and build a touch event as a plain `Event` wearing a `touches`
  property — jsdom cannot construct a `Touch`, and React's synthetic touch event only copies the
  field off the native one.
